...
class MinorSeventhTest extends AnyPropSpec with ScalaCheckPropertyChecks {
  ...
  property(
    ...
    forAll(for {
      ...
      case Some((seventh: MinorSeventh, key: MinorKey)) =>
        assert(
  ...
  property(
    ...
    forAll(minorSeventhChordGen) { seventh: MinorSeventh =>
      val triad = MinorTriad(seventh.tonic).get
